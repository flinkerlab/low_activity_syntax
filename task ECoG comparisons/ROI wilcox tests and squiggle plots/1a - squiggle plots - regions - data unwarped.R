### Compare tasks (e.g., sentence and list production) by ROI using warped data
...
for(band.loop in c('high_gamma','beta')){
  ...
  save.data.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/data/',band.loop,'/region stats/')
  if(file.exists(paste0(save.data.dir, 'ecog wilcoxon task tests.RData'))){
    ...
    # Start!
    # Time sample storage and limits
    sample.labels <- list()
    times <- list()
    time.limits <- list('locked_to_production_onset' = c('min' = -1100, 'max' = 1100),
                        'locked_to_stimulus_onset' = c('min' = -100, 'max' = 1100))
    ...
    for(lock.loop in c('locked_to_production_onset','locked_to_stimulus_onset')){
      ...
      message("Begin data set up for loops: ",band.loop,' - ', lock.loop,'. ', Sys.time())
      
      # Initialize data storage
      elec.data <- list('pn' = list(), 'sp' = list(), 'lp' = list())
      naming.trial.info <- list()
      sentence.trial.info <- list()
      listing.trial.info <- list()
      ...
      for(patient in patients){
        ...
        # Factorize
        for(lx.col.loop in 1:ncol(trial.info)){ # lx.col.loop = 1
          if(class(trial.info[,lx.col.loop])=="character"){
            trial.info[,lx.col.loop] <- factor(trial.info[,lx.col.loop])}
        }; rm(lx.col.loop)
        ...
        # Just get median.rt.samples for plotting later
        ...
        temp.elec.data <- all.data
        detach()
        
        # Trim off excess time samples
        og.sample.labels <- colnames(temp.elec.data[[1]])[(n.lx.cols+1):ncol(temp.elec.data[[1]])]
        og.times <- time.convert(og.sample.labels, "sample.labels", "times")
        remove.sample.labels <- og.sample.labels[which((og.times > time.limits[[lock.loop]]['max']) |
                                                         (og.times < time.limits[[lock.loop]]['min']))]
        remove.columns <- which(colnames(temp.elec.data[[1]]) %in% remove.sample.labels)
        temp.elec.data <- lapply(temp.elec.data, function(x){x[,-remove.columns]})
        rm(og.sample.labels, og.times, remove.columns)
        
        # Get rid of elecs you're not looking at for whatever reason
        exclude.these.elecs <- which(! names(temp.elec.data) %in% use.these.elecs)
        temp.elec.data <- temp.elec.data[-exclude.these.elecs]
        
        # Define words
        words <- c('chicken','dog','dracula','frankenstein','ninja','nurse')
        if(patient == 'Patient008'){words <- c('chicken','dog','ninja','nurse')} # ridiculously slow RTs on Drac and Frank messing things up
        
        ## Subset to just picture naming and just the narrow range of RTs
        # Create picture naming data and trial labels
        naming.trial.info[[patient]] <- just.good.pic.naming(trial.info, 
                                                             .words = words)
        elec.data[['pn']][[patient]] <- lapply(temp.elec.data, function(x){
          x <- just.good.pic.naming(x, 
                                    .words = words)
          x <- x[,(n.lx.cols+1):ncol(x)]
          return(x)
        })
        
        # Create sentence data and trial labels
        sentence.trial.info[[patient]] <- 
          just.first.word.sentence.data(trial.info,
                                        .remove.over.3000ms = ifelse(lock.loop == 'locked_to_stimulus_onset', TRUE, FALSE))
        elec.data[['sp']][[patient]] <- lapply(temp.elec.data, function(x){
          x <- just.first.word.sentence.data(x,
                                             .remove.over.3000ms = ifelse(lock.loop == 'locked_to_stimulus_onset', TRUE, FALSE))
          x <- x[,(n.lx.cols+1):ncol(x)]
          return(x)
        })
        
        # Create listing data and trial labels
        listing.trial.info[[patient]] <- 
          just.first.word.list.data(trial.info, 
                                    .remove.over.3000ms = ifelse(lock.loop == 'locked_to_stimulus_onset', TRUE, FALSE))
        elec.data[['lp']][[patient]] <- lapply(temp.elec.data, function(x){
          x <- just.first.word.list.data(x, 
                                         .remove.over.3000ms = ifelse(lock.loop == 'locked_to_stimulus_onset', TRUE, FALSE))
          x <- x[,(n.lx.cols+1):ncol(x)]
          return(x)
        })
        ...
        # Clean up
        rm(temp.elec.data)
        message(patient, ": Done reading in data. ",Sys.time())
      }; rm(patient)
      
      ## Store as one giant list of elecs across patients
      elec.data[['pn']] <- unlist(elec.data[['pn']], recursive = FALSE)
      elec.data[['sp']] <- unlist(elec.data[['sp']], recursive = FALSE)
      elec.data[['lp']] <- unlist(elec.data[['lp']], recursive = FALSE)
      
      ## Fix elec names
      for(patient in patients){ # patient = patients[1]
        names(elec.data[['pn']]) <- gsub(paste0(patient,'.'), '', names(elec.data[['pn']]), fixed = TRUE) 
        names(elec.data[['sp']]) <- gsub(paste0(patient,'.'), '', names(elec.data[['sp']]), fixed = TRUE) 
        names(elec.data[['lp']]) <- gsub(paste0(patient,'.'), '', names(elec.data[['lp']]), fixed = TRUE) 
      }; rm(patient)
      ...
      ## Get metadata
      sample.labels[[lock.loop]] <- colnames(elec.data[['pn']][[1]])
      times[[lock.loop]] <- time.convert(sample.labels[[lock.loop]], 'sample.labels', 'times')
      ...
      region.means[[lock.loop]] <- list()
      region.ses[[lock.loop]] <- list()
      ...
      for(task.loop in tasks){
        ...
        region.means[[lock.loop]][[task.loop]] <- list()
        region.ses[[lock.loop]][[task.loop]] <- list()
        region.elec.data[[task.loop]] <- list()
        ...
        for(region.loop in 1:length(region.categories)){
          ...
          region.elec.data[[task.loop]][[current.region]] <-
            bind_rows(region.elec.data[[task.loop]][[current.region]])
      ...
      for(region.loop in names(region.categories)){
        ...
        message("Beginning EGoG activity models for ",region.loop," (",which(names(region.categories) == region.loop)," of ",length(region.categories)," regions). ",Sys.time())
        ...
        for(sample.loop in sample.labels[[lock.loop]]){
          # sample.loop = sample.labels[[lock.loop]][1]
          
          ### Sentence vs. list - ONE SIDED
          current.model <-
            wilcox.test(region.elec.data$sp[[region.loop]][, sample.loop],
                        region.elec.data$lp[[region.loop]][, sample.loop], 
                        alternative = 'greater')
          ...
          ### Sentence vs. naming - ONE SIDED
          current.model <- 
            wilcox.test(region.elec.data$sp[[region.loop]][, sample.loop],
                        region.elec.data$pn[[region.loop]][, sample.loop], 
                        alternative = 'greater')
          ...
          ### List vs. naming - ONE SIDED
          current.model <- 
            wilcox.test(region.elec.data$lp[[region.loop]][, sample.loop],
                        region.elec.data$pn[[region.loop]][, sample.loop], 
                        alternative = 'greater')
  ...
  for(theme.loop in c('black','white')){
    ...
    for(lock.loop in c('locked_to_production_onset','locked_to_stimulus_onset')){
      ...
      # Save directory
      save.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/figures/',band.loop,'/by region/',alpha.label,'/3 comparisons/',theme.loop,'/')
      ...
      for(region.loop in 1:length(regions.with.data)){
        ...
        if((length(region.means[[lock.loop]]$pn[[current.region]]) > 0) & 
          ...
          plot.time.series(.y.values = list(region.means[[lock.loop]]$pn[[current.region]],
                           ...
                           .x.values = times[[lock.loop]],
                           .y.limits = y.limits[[current.region]],
                           .title = current.region,
                           .y.label = 'EGoG',
                           .x.label = paste0('Time from ',ifelse(lock.loop == 'locked_to_production_onset','speech','stimulus'),' onset (ms)'),
      ...
      # Save directory
      save.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/figures/',band.loop,'/by region/',alpha.label,'/2 comparisons/',theme.loop,'/')
      dir.create(save.dir, showWarnings = FALSE, recursive = TRUE)
      ...
      # Save directory
      save.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/figures/',band.loop,'/by region/',alpha.label,'/1 comparison/',theme.loop,'/')
      dir.create(save.dir, showWarnings = FALSE, recursive = TRUE)
      ...
      ##
      ## Just 1 comparison plotted (sentences vs. lists) - PDF FOR PUBLICATION
      ##
      
      # Save directory
      save.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/figures/',band.loop,'/by region/',alpha.label,'/1 comparison - PDFs for publication/',theme.loop,'/')
      dir.create(save.dir, showWarnings = FALSE, recursive = TRUE)
      ...
      # Save directory
      save.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/figures/',band.loop,'/by region/',alpha.label,'/1 comparison - manuscript/',theme.loop,'/')
      dir.create(save.dir, showWarnings = FALSE, recursive = TRUE)
      ...
      for(region.loop in 1:length(regions.with.data)){
        ...
        if((length(region.means[[lock.loop]]$pn[[current.region]]) > 0) & 
          ...
          if(lock.loop == 'locked_to_stimulus_onset'){
            ...
            current.x <- current.x[1:time.convert(100 + max.plot.time.ms, "times", "samples")]
      ...
      # Save directory
      save.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/figures/',band.loop,'/by task/mean/',theme.loop,'/')
      dir.create(save.dir, showWarnings = FALSE, recursive = TRUE)
    ...
    for(alpha.label in names(sentence.vs.list.sig.windows[[1]][[1]])){
      ...
      # Save directory
      save.dir <- paste0(path,'analysis/R/task ECoG comparisons/ROI wilcox tests and squiggle plots/output/1a - unwarped data/figures/',band.loop,'/by region/',alpha.label,'/1 comparison - split by median RT/',theme.loop,'/')
      dir.create(save.dir, showWarnings = FALSE, recursive = TRUE)
      ...
      for(lock.loop in c('locked_to_stimulus_onset', 'locked_to_production_onset')){
        ...
        for(region.loop in 1:length(regions.with.data)){
          ...
          if((length(region.means$locked_to_production_onset$pn[[current.region]]) > 0) & 
            ...
            current.sig.windows <- subset(current.sig.windows, duration >= 50)
            }
            ...
            plot.time.series(.y.values = current.ys,
                             ...
                             .sig.windows = list(current.sig.windows),
                             .sig.color = colors[[theme.loop]]$task_line_plots['sp','hex'],
                             .zoom = 1.2,
                             .theme = theme.loop)
            add.text.line.multiple.colors(text.segments = c('picture naming','list production','sentence production'),
