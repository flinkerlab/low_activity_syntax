### Do multiple regression on RSA values - shuffled trial labels for permutation analysis
### June 2023
...
### How many shuffles?
n.shuffle.loops <- 1000

## Save dir
elec.shuffle.ts.path <- paste0(output.path,'data/1b - elec ts - shuffled trial data/shuffle ts/')
elec.shuffle.ts.smooth.path <- paste0(output.path,'data/1b - elec ts - shuffled trial data/shuffle ts smooth/')
dir.create(elec.shuffle.ts.path, showWarnings = FALSE, recursive = TRUE)
dir.create(elec.shuffle.ts.smooth.path, showWarnings = FALSE, recursive = TRUE)

# How many shuffle loops already done?
shuffle.loops.completed <- 
  as.numeric(gsub("elec RSA multiple regression t-values for SP - shuffled data - loop ","",
                  gsub(".RData","",list.files(elec.shuffle.ts.path), fixed = TRUE), fixed = TRUE))
...
load(paste0(output.path, 'data/1a - elec ts/RSA smoothing hyperparameters.RData')) # loads "half.n.smoothing.samples" and "half.n.smoothing.window.ms"
...
for(shuffle.loop in shuffle.loops.to.do){
  ...
  start.time <- Sys.time()
  print(paste0("Beginning shuffle.loop ",shuffle.loop," (",which(shuffle.loops.to.do == shuffle.loop)," of ",length(shuffle.loops.to.do),"). ",start.time))
  
  elec.shuffle.ts.file <- paste0('elec RSA multiple regression t-values for SP - shuffled data - loop ',shuffle.loop,'.RData')
  ...
  term.stats <- 
    foreach(elec.loop = elecs.to.loop.thru$elec) %dopar% {
      ...
      # Set seed
      current.seed = 10000 + (1000 * which(elecs.to.loop.thru$elec == elec.loop)) + shuffle.loop
      ...
      ### SHUFFLE
      sp.info[[patient]] <- sp.info[[patient]][sample(1:nrow(sp.info[[patient]])),]
      ...
      ## Sentence Production
      # Set up storage
      sp.shuffle.ts <- data.frame(matrix(nrow = length(sp.sample.labels),
                                         ncol = length(sp.model.terms)))
      colnames(sp.shuffle.ts) <- sp.model.terms
      rownames(sp.shuffle.ts) <- sp.sample.labels
      ...
      for(sample.loop in sp.sample.labels){
        ...
        for(term.loop in rownames(model)[rownames(model) != "(Intercept)"]){
          sp.shuffle.ts[sample.loop, term.loop] <- model[term.loop, 't value']
        }; rm(term.loop)
      ...
      ### Smooth
      sp.shuffle.ts.smooth <- apply(sp.shuffle.ts, 2, function(x){return(smoothing(x, n.samples.pre = half.n.smoothing.samples))})
      # Remove NAs introduced by smoothing
      sp.shuffle.ts.smooth <- sp.shuffle.ts.smooth[-c(1:half.n.smoothing.samples,
                                      (nrow(sp.shuffle.ts.smooth) - half.n.smoothing.samples + 1):nrow(sp.shuffle.ts.smooth)),]
      
      ## Output
      return.stats <- list('sp.shuffle.ts' = sp.shuffle.ts,
                           'sp.shuffle.ts.smooth' = sp.shuffle.ts.smooth,
                           'elec' = elec.loop)
      return(return.stats)
  ...
  sp.shuffle.ts <- lapply(term.stats, function(x){return(x[['sp.shuffle.ts']])})
  sp.shuffle.ts.smooth <- lapply(term.stats, function(x){return(x[['sp.shuffle.ts.smooth']])})
  ...
  save(sp.shuffle.ts,
       file = paste0(elec.shuffle.ts.path, elec.shuffle.ts.file))
  save(sp.shuffle.ts.smooth,
       file = paste0(elec.shuffle.ts.smooth.path, elec.shuffle.ts.file))
  rm(sp.shuffle.ts, sp.shuffle.ts.smooth)
  gc()
  
  print(paste0("Shuffle loop duration: ", Sys.time() - start.time))
