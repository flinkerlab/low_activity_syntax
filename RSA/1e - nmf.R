...
message('Beginning loop ',include.depth.dir,' and ',include.inactive.dir,'. ',Sys.time())
...
detach()
message('...detached!')
# Add NA dataframe for Patient006 LP block (patient skipped it)
...
# Add NA dataframe for NY873 LP block
...
for(alpha.loop in alpha.hps){
  ...
  print(paste0("Beginning loop: ",include.depth.dir,' - ',include.inactive.dir,' - ',
               "with hga - ",
               alpha.loop,"."))
  
  # NMF ranks to try
  nmf.ranks.to.try <- 3:9
  ...
  nmf.results.path <- paste0(output.path,'data/1e - nmf/',
                             ...
                             alpha.loop,'/',
                             'all models/')
  nmf.results.file <- 'NMF models of RSA results and elec means.RData'
  ...
  if(file.exists(paste0(nmf.results.path, nmf.results.file))){
    ...
    ### Stack
    stacked.data[['hga']] <- bind_rows(post.stim.elec.means)
    rm(post.stim.elec.means)
    
    ## Subset to just electrodes you're using
    elecs <- names(zs.adjusted)[names(zs.adjusted) %in% elecs.with.passive.data]
    stacked.data[['hga']] <- stacked.data[['hga']][,elecs]
    
    ## Do the same transformation you did to the t-values
    stacked.data[['hga']] <- data.frame(apply(stacked.data[['hga']], 2, function(x){squish(x, 1)}))
    ...
    # diff.1st.word
    sp.1st.word <- data.frame(bind_cols(lapply(zs.adjusted, function(x){
    ...
    sp.event.semantics <- data.frame(bind_cols(lapply(zs.adjusted, function(x){
    ...
    ## Stack
    stacked.data[['sp']] <- bind_rows(list(sp.voice,
                                           sp.1st.word,
                                           sp.event.semantics))
    ...
    if(alpha.loop == "including_inactive_only_if_RSA_sig_alpha=.05_min=100ms"){
      ...
      all.sig.elecs <- unique(unlist(zs.sig.elecs[['alpha=.05_min=100ms']], use.names = FALSE))
      sp.sig.elecs.all <- intersect(names(zs.adjusted), union(all.active.elecs, all.sig.elecs))
    }
    if(alpha.loop == "including_inactive_only_if_RSA_sig_alpha=.01_min=50ms"){
      all.active.elecs <- elec.info[(elec.info$use.these.elecs == 1) &
                                      (elec.info$active == 1),]$patient_elec
      all.sig.elecs <- unique(unlist(zs.sig.elecs[['alpha=.01_min=50ms']], use.names = FALSE))
      sp.sig.elecs.all <- intersect(names(zs.adjusted), union(all.active.elecs, all.sig.elecs))
    }
  ...
  ### NMF summary plots
  save.nmf.rank.summary.fig.path <- paste0(output.path,'figures/1e - nmf/',
                                           include.depth.dir,'/',
                                           include.inactive.dir,'/',
                                           "with hga/",
                                           alpha.loop,'/',
  ...
  for(nmf.rank.loop in nmf.ranks.to.try){
    ...
    for(term.loop in names(zs.sig.elecs[[1]])){
      ...
      for(elec.loop in term.times$elec){
        ...
        if(current.alpha == 'all_elecs'){current.alpha <- "alpha=.05_min=100ms"}
        current.sig.windows <- 
    ...
    ### Save results
    current.save.data.path <- paste0(output.path,'data/1e - nmf/',
                                     include.depth.dir,'/',
                                     include.inactive.dir,'/',
                                     "with hga/",
                                     alpha.loop,'/',
    ...
    plot.grid <- c(3, nmf.rank.loop)
    save.grid.plot.path <- paste0(output.path, 'figures/1e - nmf/',
                                  include.depth.dir,'/',
                                  include.inactive.dir,'/',
                                  "with hga/",
                                  alpha.loop,'/',
